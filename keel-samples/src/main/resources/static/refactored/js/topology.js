import { state, els, selectedNode } from './state.js';
...
export function renderActiveFlows(positions) {
    const now = Date.now();
    state.activeFlows = state.activeFlows.filter((flow) => now - flow.createdAt < flow.ttlMs);
    els.topologyFlowSvg.innerHTML = state.activeFlows.map((flow) => {
    ...
    if (state.activeFlows.length) {
        requestAnimationFrame(() => renderActiveFlows(positions));
    }
}

export function applyViewport() {
    els.topologyViewport.style.transform = `translate(${state.panX}px, ${state.panY}px) scale(${state.zoom})`;
...
export function renderTopologyGraph() {
    ...
    if (!nodes.length) {
        els.topologyBaseSvg.innerHTML = "";
        els.topologyFlowSvg.innerHTML = "";
        els.topologyNodeLayer.innerHTML = `<div class="empty" style="position:absolute; inset:24px; display:grid; place-items:center;">No topology data available.</div>`;
        return;
    }
    const positions = inferLayout(nodes);
    const edges = inferEdges(nodes);
    const sNode = selectedNode();
    const selectedId = sNode ? sNode.id : null;
    els.topologyBaseSvg.innerHTML = edges.map((edge) => {
    ...
    els.topologyNodeLayer.innerHTML = nodes.map((node) => renderNode(node, positions.get(node.id))).join("");
    renderActiveFlows(positions);
    applyViewport();
}
