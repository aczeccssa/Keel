...
export class KeelDataTable extends KeelElement {
    ...
    render({ headers = [], rows = [], emptyHtml = '', silent = false, keyColumn = 0 } = {}) {
        if (rows.length === 0) {
    ...
    _diffRows(newRows, keyColumn) {
        const tbody = this.refs.wrap.querySelector('tbody');
        if (!tbody) return;
        const kc = Math.max(0, keyColumn || 0);
        const oldKeyToTr = new Map();
        Array.from(tbody.querySelectorAll('tr')).forEach(tr => {
            const key = tr.dataset.k || (tr.dataset.k = Array.from(tr.cells).slice(0, kc + 1).map(c => c.textContent.trim()).join('::') || '');
            oldKeyToTr.set(key, tr);
        });
        const newKeys = new Set();
        newRows.forEach(row => {
            const key = row.slice(0, kc + 1).map(c => (c || '').replace(/<[^>]+>/g, '').trim()).join('::');
            newKeys.add(key);
