...
object PluginJvmFrameCodec {
    fun write(socketChannel: SocketChannel, payload: String) {
        val bytes = payload.toByteArray(Charsets.UTF_8)
        require(bytes.size <= PluginJvmLimits.MAX_FRAME_BYTES) {
            "JVM frame too large: ${bytes.size}"
        }
    ...
    fun read(input: InputStream): String {
        ...
        require(size in 0..PluginJvmLimits.MAX_FRAME_BYTES) { "Invalid JVM frame size: $size" }
    ...
    private fun readFully(input: InputStream, target: ByteArray) {
        ...
        while (offset < target.size) {
            ...
            if (read < 0) {
                throw EOFException("Unexpected EOF while reading JVM frame")
            }
            offset += read
        }
    }
